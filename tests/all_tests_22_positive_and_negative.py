...
def test_add_new_pet_simple(name, animal_type, age): # тест_добавить_нового_питомца_просто
   ...
   if name == '' or animal_type == '' or is_age_valid():
       assert pytest.status == 400 # при коде 400 тесты валятся. При коде 200 - проходят => проблема с сайтом
   else:
       assert pytest.status == 200
